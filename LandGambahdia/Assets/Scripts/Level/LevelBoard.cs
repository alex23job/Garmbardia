            ...
            foreach (GameObject prefab in _buildingPrefabs)
            {
                ...
                if (bc != null)
                {
                    if (bc.BuildingID == buildID)
                    {
                        ...
                        {
                            _levelControl.ViewError("Ќедостаточно денег дл€ постройки !");
                            return null;
                        }                        
                        ...
                        {
                            _levelControl.ViewError("“ип местности не соответствует виду постройки !");
                            return null;
                        }
            ...
            for (i = 0; i < terrain.Length; i++)
            {
                ...
                for (j = 0; j < _landTails.Length; j++)
                {
                    if (_landTails[j].CmpID(num % 90, (num >= 90 ? 9 : 0)))
                    {
                        ...
                        GameObject tail;
                        if (num == 96)  //  дорогу из схемы уровн€ замен€ем на траву и "здание" дерев€нна€ дорога
                        {
